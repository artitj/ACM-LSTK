...
vtkStandardNewMacro(vtkCutPlaneWidget);

vtkCxxSetObjectMacro(vtkCutPlaneWidget, PlaneProperty, vtkProperty);
vtkCxxSetObjectMacro(vtkCutPlaneWidget, SelectedPlaneProperty, vtkProperty);
vtkCxxSetObjectMacro(vtkCutPlaneWidget, CursorProperty, vtkProperty);
vtkCxxSetObjectMacro(vtkCutPlaneWidget, MarginProperty, vtkProperty);
vtkCxxSetObjectMacro(vtkCutPlaneWidget, TexturePlaneProperty, vtkProperty);
vtkCxxSetObjectMacro(vtkCutPlaneWidget, ColorMap, vtkImageMapToColors);

//----------------------------------------------------------------------------
vtkCutPlaneWidget::vtkCutPlaneWidget() : vtkPolyDataSourceWidget()
{
	m_Surface = nullptr;
  this->State = vtkCutPlaneWidget::Start;
  this->EventCallbackCommand->SetCallback(vtkCutPlaneWidget::ProcessEvents);
  ...
  this->MarginSizeY              = 0.05;
	...
	this->SurfaceOutlineMapper = vtkPolyDataMapper::New();
	this->CutPlane = vtkPlane::New();
	...
	this->SurfaceOutlineActor->SetProperty(surfaceOutlineProperty);
	this->SurfaceOutlineActor->SetMapper(SurfaceOutlineMapper);
	SurfaceOutlineMapper->ScalarVisibilityOff();
	this->SurfaceOutlineActor->SetVisibility(false);
	this->SurfaceCutter = vtkCutter::New();
	this->SurfaceCutter->GenerateCutScalarsOff();
	this->SurfaceOutlineMapper->SetInputConnection(this->SurfaceCutter->GetOutputPort());
	this->SurfaceCutter->SetCutFunction(this->CutPlane);
  ...
  this->LeftButtonAction = vtkCutPlaneWidget::VTK_CURSOR_ACTION;
  this->MiddleButtonAction = vtkCutPlaneWidget::VTK_SLICE_MOTION_ACTION;
  this->RightButtonAction = vtkCutPlaneWidget::VTK_WINDOW_LEVEL_ACTION;

  // Set up modifiers

  this->LeftButtonAutoModifier = vtkCutPlaneWidget::VTK_NO_MODIFIER;
  this->MiddleButtonAutoModifier = vtkCutPlaneWidget::VTK_NO_MODIFIER;
  this->RightButtonAutoModifier = vtkCutPlaneWidget::VTK_NO_MODIFIER;

  this->LastButtonPressed = vtkCutPlaneWidget::VTK_NO_BUTTON;

  this->TextureVisibility = 1;
}

//----------------------------------------------------------------------------
vtkCutPlaneWidget::~vtkCutPlaneWidget()
{
  this->PlaneOutlineActor->Delete();
  this->PlaneOutlinePolyData->Delete();
  this->PlaneSource->Delete();

	this->SurfaceOutlineMapper->Delete();
	this->CutPlane->Delete();
	this->SurfaceOutlineActor->Delete();
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetTextureVisibility(vtkTypeBool vis)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetEnabled(int enabling)
{
  ...
  if ( enabling ) //----------------------------------------------------------
  {
    ...
    this->CurrentRenderer->AddViewProp(this->TextActor);
		...
		// Add the surface outlines
		this->CurrentRenderer->AddViewProp(this->SurfaceOutlineActor);
  ...
  else //disabling----------------------------------------------------------
  {
    ...
    this->CurrentRenderer->RemoveViewProp(this->TextActor);
		...
		this->CurrentRenderer->RemoveViewProp(this->SurfaceOutlineActor);
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::ProcessEvents(vtkObject* vtkNotUsed(object),
                                        unsigned long event,
                                        void* clientdata,
                                        void* vtkNotUsed(calldata))
{
  vtkCutPlaneWidget* self =
    reinterpret_cast<vtkCutPlaneWidget *>( clientdata );

  self->LastButtonPressed = vtkCutPlaneWidget::VTK_NO_BUTTON;

  //okay, let's do the right thing
  switch ( event )
  {
    case vtkCommand::LeftButtonPressEvent:
      self->LastButtonPressed = vtkCutPlaneWidget::VTK_LEFT_BUTTON;
    ...
    case vtkCommand::MiddleButtonPressEvent:
      self->LastButtonPressed = vtkCutPlaneWidget::VTK_MIDDLE_BUTTON;
    ...
    case vtkCommand::RightButtonPressEvent:
      self->LastButtonPressed = vtkCutPlaneWidget::VTK_RIGHT_BUTTON;
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnChar()
{
  vtkRenderWindowInteractor *i = this->Interactor;

	if (i->GetKeyCode() == 'k' || i->GetKeyCode() == 'K')
	{
		bool flag = !(i->GetControlKey());
		this->SurfaceOutlineActor->SetVisibility(flag);
		this->m_SurfaceActor->SetVisibility(!flag);
		this->Interactor->Render();
	}
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::AddObservers(void)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetInteraction(vtkTypeBool interact)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::BuildRepresentation()
{
  ...
  this->PlaneOutlinePolyData->Modified();
	...
	this->CutPlane->SetOrigin(this->PlaneSource->GetOrigin());
	this->CutPlane->SetNormal(this->PlaneSource->GetNormal());
	//if (this->PlaneProperty)
	//	this->SurfaceOutlineActor->GetProperty()->SetColor(
	//		this->PlaneProperty->GetColor());
	if (this->m_Surface)
	{
		this->CutPlane->Modified();
		this->SurfaceCutter->Update();
	}
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::HighlightPlane(int highlight)
{
  if ( highlight )
  {
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnLeftButtonDown()
{
  switch (this->LeftButtonAction)
  {
    case vtkCutPlaneWidget::VTK_CURSOR_ACTION:
      this->StartCursor();
      break;
    case vtkCutPlaneWidget::VTK_SLICE_MOTION_ACTION:
      this->StartSliceMotion();
      break;
    case vtkCutPlaneWidget::VTK_WINDOW_LEVEL_ACTION:
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnLeftButtonUp()
{
  switch (this->LeftButtonAction)
  {
    case vtkCutPlaneWidget::VTK_CURSOR_ACTION:
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnMiddleButtonDown()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnMiddleButtonUp()
{
  switch (this->MiddleButtonAction)
  {
    case vtkCutPlaneWidget::VTK_CURSOR_ACTION:
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnRightButtonDown()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnRightButtonUp()
{
  switch (this->RightButtonAction)
  {
    case vtkCutPlaneWidget::VTK_CURSOR_ACTION:
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::StartCursor()
{
  ...
  {
    this->State = vtkCutPlaneWidget::Outside;
    return;
  }
  ...
  {
    this->State = vtkCutPlaneWidget::Cursoring;
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::StopCursor()
{
  if ( this->State == vtkCutPlaneWidget::Outside ||
       this->State == vtkCutPlaneWidget::Start )
  {
    return;
  }
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::StartSliceMotion()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  {
    this->State = vtkCutPlaneWidget::Pushing;
    this->HighlightPlane(1);
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::StopSliceMotion()
{
  if ( this->State == vtkCutPlaneWidget::Outside ||
       this->State == vtkCutPlaneWidget::Start )
  {
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::StartWindowLevel()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  {
    this->State = vtkCutPlaneWidget::WindowLevelling;
    this->HighlightPlane(1);
    this->ActivateText(1);
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::StopWindowLevel()
{
  if ( this->State == vtkCutPlaneWidget::Outside ||
       this->State == vtkCutPlaneWidget::Start )
  {
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::OnMouseMove()
{
  ...
  if ( this->State == vtkCutPlaneWidget::WindowLevelling )
  {
    this->WindowLevel(X,Y);
    this->ManageTextDisplay();
  }
  else if ( this->State == vtkCutPlaneWidget::Pushing )
  {
  ...
  else if ( this->State == vtkCutPlaneWidget::Spinning )
  {
  ...
  else if ( this->State == vtkCutPlaneWidget::Rotating )
  {
  ...
  else if ( this->State == vtkCutPlaneWidget::Scaling )
  {
  ...
  else if ( this->State == vtkCutPlaneWidget::Moving )
  {
  ...
  else if ( this->State == vtkCutPlaneWidget::Cursoring )
  {
    this->UpdateCursor(X,Y);
    this->ManageTextDisplay();
  }
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::WindowLevel(int X, int Y)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::InvertTable()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetWindowLevel(double window, double level, int copy)
{
  if ( copy )
  {
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetWindowLevel(double wl[2])
{
...
//----------------------------------------------------------------------------
int vtkCutPlaneWidget::GetCursorData(double xyzv[4])
{
  if ( this->State != vtkCutPlaneWidget::Cursoring  || \
    this->CurrentImageValue == VTK_DOUBLE_MAX )
  {
    return 0;
  }
...
//----------------------------------------------------------------------------
int vtkCutPlaneWidget::GetCursorDataStatus()
{
  if ( this->State != vtkCutPlaneWidget::Cursoring  || \
    this->CurrentImageValue == VTK_DOUBLE_MAX )
  {
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::ManageTextDisplay()
{
  if ( !this->DisplayText )
  {
    return;
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::Push(double *p1, double *p2)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::CreateDefaultProperties()
{
  if ( ! this->PlaneProperty )
  {
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::PlaceWidget(double bds[6])
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetPlaneOrientation(int i)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetInputConnection(vtkAlgorithmOutput* aout)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::UpdatePlane()
{
...
//----------------------------------------------------------------------------
vtkImageData* vtkCutPlaneWidget::GetResliceOutput()
{
  if ( ! this->Reslice )
  {
    return nullptr;
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetResliceInterpolate(int i)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetPicker(vtkAbstractPropPicker* picker)
{
...
//------------------------------------------------------------------------------
void vtkCutPlaneWidget::RegisterPickers()
{
...
//----------------------------------------------------------------------------
vtkLookupTable* vtkCutPlaneWidget::CreateDefaultLookupTable()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetSurface(vtkPolyData *s)
{
	this->SurfaceCutter->SetInputData(s);
	m_Surface = s;
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetLookupTable(vtkLookupTable* table)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetSlicePosition(double position)
{
  double amount = 0.0;
...
//----------------------------------------------------------------------------
double vtkCutPlaneWidget::GetSlicePosition()
{
  double planeOrigin[3];
  this->PlaneSource->GetOrigin( planeOrigin);
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetSliceIndex(int index)
{
  if ( !this->Reslice )
  {
      return;
...
//----------------------------------------------------------------------------
int vtkCutPlaneWidget::GetSliceIndex()
{
  if ( ! this->Reslice )
  {
    return 0;
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::ActivateCursor(int i)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::ActivateMargins(int i)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::ActivateText(int i)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::UpdateCursor(int X, int Y )
{
  if ( !this->ImageData )
  {
    return;
...
//----------------------------------------------------------------------------
int vtkCutPlaneWidget::UpdateContinuousCursor(double *q)
{
  double tol2;
  vtkCell *cell;
  vtkPointData *pd;
...
//----------------------------------------------------------------------------
int vtkCutPlaneWidget::UpdateDiscreteCursor(double *q)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetOrigin(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetOrigin(double xyz[3])
{
...
//----------------------------------------------------------------------------
double* vtkCutPlaneWidget::GetOrigin()
{
  return this->PlaneSource->GetOrigin();
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetOrigin(double xyz[3])
{
  this->PlaneSource->GetOrigin(xyz);
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetPoint1(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetPoint1(double xyz[3])
{
...
//----------------------------------------------------------------------------
double* vtkCutPlaneWidget::GetPoint1()
{
  return this->PlaneSource->GetPoint1();
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetPoint1(double xyz[3])
{
  this->PlaneSource->GetPoint1(xyz);
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetPoint2(double x, double y, double z)
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetPoint2(double xyz[3])
{
...
//----------------------------------------------------------------------------
double* vtkCutPlaneWidget::GetPoint2()
{
  return this->PlaneSource->GetPoint2();
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetPoint2(double xyz[3])
{
  this->PlaneSource->GetPoint2(xyz);
}

//----------------------------------------------------------------------------
double* vtkCutPlaneWidget::GetCenter()
{
  return this->PlaneSource->GetCenter();
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetCenter(double xyz[3])
{
  this->PlaneSource->GetCenter(xyz);
}

//----------------------------------------------------------------------------
double* vtkCutPlaneWidget::GetNormal()
{
  return this->PlaneSource->GetNormal();
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetNormal(double xyz[3])
{
  this->PlaneSource->GetNormal(xyz);
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetPolyData(vtkPolyData *pd)
{
  pd->ShallowCopy(this->PlaneSource->GetOutput());
}

//----------------------------------------------------------------------------
vtkPolyDataAlgorithm *vtkCutPlaneWidget::GetPolyDataAlgorithm()
{
  return this->PlaneSource;
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::UpdatePlacement(void)
{
  this->UpdatePlane();
  this->UpdateMargins();
  this->BuildRepresentation();
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::SetTextProperty(vtkTextProperty* tprop)
{
  this->TextActor->SetTextProperty(tprop);
}

//----------------------------------------------------------------------------
vtkTextProperty* vtkCutPlaneWidget::GetTextProperty()
{
  return this->TextActor->GetTextProperty();
}

//----------------------------------------------------------------------------
vtkTexture* vtkCutPlaneWidget::GetTexture()
{
  return this->Texture;
}

//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetVector1(double v1[3])
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GetVector2(double v2[3])
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::AdjustState()
{
  int *auto_modifier = nullptr;
  switch (this->LastButtonPressed)
  {
    case vtkCutPlaneWidget::VTK_LEFT_BUTTON:
      auto_modifier = &this->LeftButtonAutoModifier;
      break;
    case vtkCutPlaneWidget::VTK_MIDDLE_BUTTON:
      auto_modifier = &this->MiddleButtonAutoModifier;
      break;
    case vtkCutPlaneWidget::VTK_RIGHT_BUTTON:
  ...
  if (this->Interactor->GetShiftKey() ||
      (auto_modifier &&
       (*auto_modifier & vtkCutPlaneWidget::VTK_SHIFT_MODIFIER)))
  {
    this->State = vtkCutPlaneWidget::Scaling;
    return;
  }
  ...
  if (this->Interactor->GetControlKey() ||
      (auto_modifier &&
       (*auto_modifier & vtkCutPlaneWidget::VTK_CONTROL_MODIFIER)))
  {
    this->State = vtkCutPlaneWidget::Moving;
  }
  else
  {
    if (this->MarginSelectMode >= 0 && this->MarginSelectMode < 4)
    {
      this->State = vtkCutPlaneWidget::Spinning;
      return;
    }
    ...
    {
      this->State = vtkCutPlaneWidget::Rotating;
    }
  }
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::Spin(double *p1, double *p2)
{
  // Disable cursor snap
  //
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::Rotate(double *p1, double *p2, double *vpn)
{
  // Disable cursor snap
  //
  this->PlaneOrientation = 3;
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GeneratePlaneOutline()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GenerateTexturePlane()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GenerateMargins()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GenerateCursor()
{
  // Construct initial points
  //
  vtkPoints* points = vtkPoints::New(VTK_DOUBLE);
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::GenerateText()
{
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::UpdateMargins()
{
  double v1[3];
  this->GetVector1(v1);
  double v2[3];
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::Translate(double *p1, double *p2)
{
  // Get the motion vector
  //
  double v[3];
...
//----------------------------------------------------------------------------
void vtkCutPlaneWidget::Scale(double *p1, double *p2,
...
void vtkCutPlaneWidget::SetSegmentationRenderMode(vtkCutPlaneWidget::SegmentationRenderMode s)
{
	const bool flag = (s == vtkCutPlaneWidget::SegmentationRenderMode::Outline);
	this->SurfaceOutlineActor->SetVisibility(flag);
	this->m_SurfaceActor->SetVisibility(!flag);
}
